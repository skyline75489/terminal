...
DxFontInfo::DxFontInfo(const DxFontInfo& other) :
    _familyName(other.GetFamilyName()),
    _weight(other.GetWeight()),
    _style(other.GetStyle()),
    _stretch(other.GetStretch())
{
}
...
void DxFontInfo::SetFamilyName(const std::wstring_view familyName) noexcept
{
    _familyName = familyName;
}

DWRITE_FONT_WEIGHT DxFontInfo::GetWeight() const
{
    return _weight;
}
...
DWRITE_FONT_STYLE DxFontInfo::GetStyle() const
{
    return _style;
}
...
DWRITE_FONT_STRETCH DxFontInfo::GetStretch() const
{
    return _stretch;
}
...
[[nodiscard]] Microsoft::WRL::ComPtr<IDWriteFontFace1> DxFontInfo::ResolveFontFaceWithFallback(IDWriteFactory1* dwriteFactory,
...
[[nodiscard]] ::Microsoft::WRL::ComPtr<IDWriteTextFormat> DxFontInfo::ToTextFormat(IDWriteFactory1* dwriteFactory, const float fontSize, const std::wstring_view localeName)
{
...
// Return Value:
// - Smart pointer holding interface reference for queryable font data.
[[nodiscard]] Microsoft::WRL::ComPtr<IDWriteFontFace1> DxFontInfo::_FindFontFace(IDWriteFactory1* dwriteFactory, std::wstring& localeName)
{
